...
dmsq23_0 = 0.0024 # ?
xi_0 = 1.3 # Cross section linear co-efficient
L = 295 # km
...
def parabolic_minimisation_old(funct, X, param_minimised, function_parameters, convergence_condition, max_iterations = 1e6):
    ...
    X : TYPE
        DESCRIPTION.
    param_minimised : TYPE
        DESCRIPTION.
    function_parameters : TYPE
        DESCRIPTION.
    convergence_condition : TYPE
        DESCRIPTION.
...
def HessianCDA_2D(u, fun, funct_params ={}, deltas = None):
...
class Dataset:
    def __init__(self, fp, energy_range = None, L = 295):
        ...
        self.bin_energies = self.bins + self.bin_size/2
        self.L = L
    ...
    def flux_event_histogram(self, gridlines = True):
        ...
        fig, ax = plt.subplots()
        standard_axes_settings(ax, infile_figparams)
        ax.bar(self.bins, height = self.event_data, width = self.bin_size, bottom = 0, align = 'center', color = 'lightblue', edgecolor ='k', linewidth = 0, label = 'Observed Event Rate') 
        
        ax.bar(self.bins, height = self.flux_data, width = self.bin_size, bottom = self.event_data, align = 'center', color = 'firebrick', edgecolor ='k', linewidth = 0, label = 'Simulated Unoscillated Event Rate') 
    ...
    def flux_event_histogram_line(self, gridlines = True):
        """
        Generate histogram showing both observed data and predicted unoscillated flux
        ...
        """
        fig, ax = plt.subplots()
        # fig = plt.figure(figsize = (1.618*fig_w,fig_w), dpi = 50)
        # ax = fig.add_subplot()        
        standard_axes_settings(ax, infile_figparams)
        ax.bar(self.bins, height = self.event_data, width = self.bin_size, bottom = 0, align = 'center', color = 'powderblue', edgecolor ='k', linewidth = .4) 
        
        ax2 = ax.twinx()
        standard_axes_settings(ax2, infile_figparams)
        cc = 'r'
        ax2.plot(self.bins, self.flux_data, ls = '--', color = cc, marker = None, lw = 3)
        ax2.grid(b=True, which='major', axis='both', c='darksalmon', ls=':')

        ax2.tick_params(axis='y', labelcolor=cc)
        ax.tick_params(axis='y', labelcolor='k')
        ax.set_xlabel(U_E)
        ax2.set_ylabel(r'Unoscillated Predicted Event Rate', color = cc)
        ax.set_ylabel(U_observed_muon_events) 
        if not gridlines:
            ax.grid(b=False, which='both', axis='both', c='None', ls = '--', linewidth =1)
        ax.set_ylim(0, 24.15)
        ax.set_xlim(0, 10.475)
        ax2.set_ylim(0, 160)
        ax2.set_xlim(0, 10.475)
        return [ax, ax2]
    
    
    def lamda_u(self, thetas, delta_msqs, xis, bin_energies = None):
        """
        ...
        thetas : np.float64
            mixing angle
        delta_msqs : np.float64
            deltamsq parameter
        xis : np.float64
            cross section linear co-eff 
        bin_energies : np.ndarray, optional
            The default is None. Energies to use for bins. If none, uses calculated mean bin energies for dataset. 
        ...
        lamdas = (np.abs(xis)*xi_0)*np.multiply(bin_energies, np.multiply(self.flux_data, P_noosc_muon(bin_energies, self.L, thetas, delta_msqs)))
        return lamdas
        
    def compare_sim_real_histogram(self, thetas, deltamsqs, xis, stacked = True, gridlines = True, bin_energies = None):
        """
        ...
        gridlines : bool, optional
            The default is True.
        bin_energies : None or (self.num_bins, ) array
            Mean bin energies to calculate lamda(u)
        ...
        """
        ll = self.lamda_u(thetas, deltamsqs, xis, bin_energies)
        fig, axl = plt.subplots()
        standard_axes_settings(axl, infile_figparams)
        axl.bar(self.bins, height = self.event_data, width = self.bin_size, bottom = 0, align = 'center', zorder = 10, color = 'lightblue', edgecolor ='k', linewidth = .3, label = 'Observed Data') 
        if stacked:
            bottom = self.event_data
        else: 
            bottom = 0
        axl.bar(self.bins, height = ll, width = self.bin_size, bottom = bottom, align = 'center', color = 'indianred', edgecolor ='k', linewidth = 0, label = 'Simulated Oscillated Rate') 
    ...
    def compare_sim_real(self, thetas, deltamsqs, xis, stacked = True, gridlines = True, bin_energies = None):
        """
        ...
        fig, ax = plt.subplots()
        standard_axes_settings(ax, infile_figparams)
        ax.bar(self.bins, height = self.event_data, width = self.bin_size, bottom = 0, align = 'center', color = 'lightblue', edgecolor ='k', linewidth = 0.4, label = 'Observed Data') 
        cc = 'r'
        ax.plot(self.bins, ll, ls = '--', color = cc, marker = None, lw = 3, label = 'Simulated Oscillated Rate')
        ax.set_xlabel(U_E)
        ax.set_ylabel(r'Number of $\nu_{\mu}$ Events') 
        if not gridlines:
            ax.grid(b=False, which='both', axis='both', c='None', ls = '--', linewidth =2)
        leg = ax.legend(fontsize = 20, loc='best', markerfirst = True, frameon = True)
        leg.get_frame().set_edgecolor('k')
        leg.get_frame().set_facecolor('w')
        return ax
    
    def NLL_params(self, thetas, deltamsqs, xis, bin_energies = None):
        """
        ...
        deltamsqs : (N, ) array or float/int
            deltasqm values 
        xis : (N, ) array or float/int
            cross-section linear coeff values
        bin_energies : None or (self.num_bins, ) array
            Mean bin energies to calculate lamda(u)
        ...
        if isinstance(deltamsqs, (float, np.float64, int, np.int32)):
            deltamsqs = [deltamsqs]
        if isinstance(xis, (float, np.float64, int, np.int32)):
            xis = [xis]
            
            
        if len(thetas) != len(deltamsqs) or len(thetas) != len(xis):
            raise TypeError(f'theta, deltamsq and xi arrays must be of same length, of length {len(thetas)}, {len(deltamsqs)}, {len(xis)}')
        
        params = np.array([thetas,deltamsqs, xis])    
        N = params.shape[1] # number of different parameter values to evalute
        NLL = np.empty(N)
        for i in range(N):
            lamdas = self.lamda_u(params[0,i], params[1,i], params[2, i], bin_energies = bin_energies)
    ...
    def thetamin_stddev_NLL(self, mintheta, deltamsq = 1, xi = 1, bin_energies = None):
        """
        ...
        mintheta : TYPE
            DESCRIPTION.
        deltamsq : TYPE, optional
            DESCRIPTION. The default is 1.
        xi : TYPE, optional
            DESCRIPTION. The default is 1.
        bin_energies : TYPE, optional
            DESCRIPTION. The default is None.
        ...
        c = 0
        z = 1
        NLL_min = self.NLL_params([mintheta], [deltamsq], [xi], bin_energies)
        evals = 30
        ...
        ys = self.NLL_params(a, np.full(evals, deltamsq), np.full(evals, xi), bin_energies)
        
        while c < 4:
            q = np.abs(ys - (NLL_min+0.5)).argmin()
            thp = a[q]
            # NLL_05p = self.NLL_params([thp], [deltamsq], [xi], bin_energies)
        ...
        while c < 4:
            q = np.abs(ys - (NLL_min+0.5)).argmin()
            thm = a[q]
            # NLL_05m = self.NLL_params([thm], [deltamsq], [xi], bin_energies)
    ...
    def grad_NLL(self, params, deltas, bin_energies = None):
        """
        ...
        ----------
        params : (3,) array
            (theta, dmsq, xi) at which to calculate grad
        deltas : (3,) array or np.float64
        ...
        """
        Ndim = len(params)
        U = {'thetas':params[0], 'deltamsqs':params[1], 'xis':params[2]}
        
        if deltas is None:
            deltas = np.full(Ndim, 0.1)
        elif isinstance(deltas, (int, np.float64, np.int32, float)):
            deltas = np.full(Ndim, deltas)
        elif isinstance(deltas, np.ndarray) and len(deltas) == Ndim:
            pass
        ...
        for i, key in enumerate(U):
            p = U.copy()
            p[key] += deltas[i]
            # print(f'self.NLL_params({params[i]}+{deltas[i]}, {p}, {bin_energies}) - self.NLL_params({params}, {bin_energies})')
            dd = (self.NLL_params(bin_energies = bin_energies, **p) - self.NLL_params(bin_energies = bin_energies, **U))/deltas[i]
            grad[i] = dd
        
        # diff_th = (self.NLL_params(params[0]+deltas[0], params[1], params[2], bin_energies) - self.NLL_params(*params), bin_energies)/deltas[0]
        # diff_dm = (self.NLL_params(params[0], params[1]+deltas[1], params[2], bin_energies) - self.NLL_params(*params, bin_energies))/deltas[1]
        # grad = np.array([diff_th, diff_dm])
        
        return grad
    
    def parabolic_minimisation_NLL_deltamsq(self, X, theta, xi, conv_condition, max_iter = None, bin_energies = None):
        """
        ...
        xi : float or int
            Value of xi at which to evaluate NLL
        ...
        """
        NLL_args = {'thetas':theta, 'xis':xi, 'bin_energies':bin_energies}
        if max_iter is None:
            max_iter = 1e6
        x3, epsilon, c, parabola  = parabolic_minimisation(self.NLL_params, X, 'deltamsqs', NLL_args, conv_condition, max_iterations = max_iter)
        return x3, epsilon, c, parabola
    
    def parabolic_minimisation_NLL_theta(self, X, deltamsq, xi, conv_condition, max_iter = None, bin_energies = None):
        """
        ...
        NLL_args = {'deltamsqs':deltamsq, 'xis':xi, 'bin_energies':bin_energies}
        if max_iter is None:
            max_iter = 1e6
        x3, epsilon, c, parabola  = parabolic_minimisation(self.NLL_params, X, 'thetas', NLL_args, conv_condition, max_iterations = max_iter)
        return x3, epsilon, c, parabola

    def parabolic_minimisation_NLL_xi(self, X, theta, deltamsq, conv_condition, max_iter = None, bin_energies = None):
        """
        Finds minimum value of NLL function using Parabolic minimisation in the xi direction

        Parameters
        ----------
        X :  (3,) array
            3 xi points with which to start minimisation
        theta : float or int
            Value of theta at which to evaluate NLL
        deltamsq : float or int
            Value of deltamsq at which to evaluate NLL
        ...
        c : int
            Number of iterations.
        parabola : np.array shape (2, 3)
            (xi, NLL points) for final + 1th iteration .

        """

        NLL_args = {'deltamsqs':deltamsq, 'thetas':theta, 'bin_energies':bin_energies}
        if max_iter is None:
            max_iter = 1e6
        x3, epsilon, c, parabola  = parabolic_minimisation(self.NLL_params, X, 'xis', NLL_args, conv_condition, max_iterations = max_iter)
        return x3, epsilon, c, parabola
